                   ...
                   <div class="col-md-8 offset-md-2">
                     <div class="card">
                       ...
                       <img src="img/properties/<%=p.getImg()%>"  style="max-height:190px; max-width:100%;width:auto;" class="card-img-top m-2" alt="...">
                          <h4>Property Details of property-<%=p.getPid() %></h4>
                       </div>
                       <div class="card-body">
                        <p> Type:<%=p.getType() %></p>
                        <p>Sqfeet:<%=p.getSquarefeet() %> sqft</p>
                        <p>Ownerid:<%=p.getOwnerid() %></p>
                        <p>Price:<%=p.getPrice() %></p>
                        <p>Address:<%=p.getStreet()%>, <%=p.getCity() %>, <%=p.getState() %> </p>
                        <p>Pincode:<%=p.getPincode() %></p>
                        <p>Number of rooms:<%=p.getBedroom() %></p>
                        <p>Number of bathrooms:<%=p.getBathroom() %></p>
                        <p>Balcony: <%if(p.getBalcony()==1){ %>Present<%}else{ %>Not present<%} %></p>
                        <p>Backyard: <%if(p.getBackyard()==1){ %>Present<%}else{ %>Not present<%} %></p>
                        <p>Pool: <%if(p.getPool()==1){ %>Present<%}else{ %>Not present<%} %></p>
                        <p>Garage: <%if(p.getGarage()==1){ %>Present<%}else{ %>Not present<%} %></p>
                        <p>Description: <%=p.getText()%></p>
                     </div>
                   </div>
                 </div>
             </div>
